...
public class KafkaTestSniffer<
    ...
    public KafkaTestSniffer(Shipper<HackitTuple<K, T>, SentType, SenderObj, ReceiverObj> shipper) {
    ...
    public Iterator<HackitTuple<K, T>> apply(HackitTuple<K, T> tHackTuple) {
        ...
        if (tHackTuple.getHeader().isSendOut()) {
            ...
            System.out.println("Sent " + (f-s));
            if (tHackTuple.hasCallback()) {
                s = System.currentTimeMillis();
                Iterator<HackitTuple<K, T>> injection = this.shipper.getNexts();
                f = System.currentTimeMillis();
                System.out.println("Received " + (f-s));
    ...
    public KafkaTestSniffer<K, T, SentType, SenderObj, ReceiverObj> setShipper(Shipper<HackitTuple<K, T>,
    ...
    public KafkaTestSniffer<K, T, SentType, SenderObj, ReceiverObj> setHackItSniff(Sniff<HackitTuple<K, T>> hackItSniff) {
        this.hackItSniff = hackItSniff;
        return this;
    }


    public KafkaTestSniffer<K, T, SentType, SenderObj, ReceiverObj> setHackItCloner(Cloner<HackitTuple<K, T>, SentType> hackItCloner) {
    ...
    public void setAction(String command) {
        switch (command) {
            case "tuple": {
                ...
                tag.add(new PauseTag());
                tag.add(new DebugTag());
                preList.add(tag);
                break;
            }
            case "operator": {
