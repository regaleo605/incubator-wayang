...
public class JavaTestSniffOperator<InputType,OutputType>
        extends TestSniffOperator<InputType,OutputType>
        implements JavaExecutionOperator{

    public JavaTestSniffOperator(DataSetType<InputType> inputType, DataSetType<OutputType> outputType,
                             FlatMapDescriptor<InputType, OutputType> functionDescriptor) {
        super(functionDescriptor, inputType, outputType);
    }

    public JavaTestSniffOperator(TestSniffOperator<InputType, OutputType> that) {
        super(that);
    }
                ...
                ((JavaChannelInstance) inputs[0]).<InputType>provideStream().flatMap(dataQuantum ->
                        StreamSupport.stream(
                                Spliterators.spliteratorUnknownSize(
                                        sniffer.apply((HackitTuple) dataQuantum),
                                        Spliterator.ORDERED),
                                false
                        )
                )
